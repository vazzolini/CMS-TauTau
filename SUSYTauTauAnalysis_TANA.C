...
void SUSYTauTauAnalysis::Loop(Int_t isDetailedHad)
{
  ...
  /// VIR ----------------------
    // -- The new file with the custom tree to be written
    //TFile *n = new TFile("new.root", "RECREATE"); 
    //TTree* tt = new TTree("tt","tt");

    // -- The variables you want in the new tree
    //  double npcms, nbla;
    //  tt->Branch("nTauEvents", &nTauEvents,"nTauEvents/D");
    //Int_t nCascade =0;
    //Int_t nTauEvents = 0;
    //Int_t nSUSYTaus = 0;
    //short TauCounter[4] = {0, 0, 0, 0};
  ...
  /* // -- The file with the big tree
    ...
    // -- Get the needed variables from the old tree
    ...
    oldtree->SetBranchAddress("nMc", &nMc, &b_nMc);
    oldtree->SetBranchAddress("pMc", pMc, &b_pMc);
    oldtree->SetBranchAddress("massMc", massMc, &b_massMc);
    oldtree->SetBranchAddress("thetaMc", thetaMc, &b_thetaMc);
    oldtree->SetBranchAddress("etaMc", etaMc, &b_etaMc);
    oldtree->SetBranchAddress("phiMc", phiMc, &b_phiMc);
    oldtree->SetBranchAddress("energyMc", energyMc, &b_energyMc);
    oldtree->SetBranchAddress("idMc", idMc, &b_idMc);
    oldtree->SetBranchAddress("mothMc", mothMc, &b_mothMc);
    oldtree->SetBranchAddress("nDauMc", nDauMc, &b_nDauMc);
    oldtree->SetBranchAddress("statusMc", statusMc, &b_statusMc);
    oldtree->SetBranchAddress("xMc", xMc, &b_xMc);
    oldtree->SetBranchAddress("yMc", yMc, &b_yMc);
    oldtree->SetBranchAddress("zMc", zMc, &b_zMc);
  */    
      /////---------------------------------------
	...
	std::cout << "There are " << TauHadCounter[3] << " events with tau -> a1 nu" << endl;
	std::cout << "There are " << TauHadCounter[4] << " events with tau -> a1 nu" << endl;
